...
globalThis.__dirname = __bannerPath.dirname(globalThis.__filename);
  ...
  // pdfkit's standard fonts (Helvetica, etc.) are loaded from .afm files
  // via a runtime fs.readFileSync relative to __dirname, which esbuild's
  // bundling can't see or inline. Copy the data files next to the bundle
  // so that lookup still resolves after bundling.
  const pdfkitRequire = createRequire(import.meta.url);
  let pdfkitDataDir;
  try {
    pdfkitDataDir = path.resolve(pdfkitRequire.resolve("pdfkit/js/pdfkit.js"), "../data");
  } catch {
    pdfkitDataDir = null;
  }
  if (pdfkitDataDir) {
